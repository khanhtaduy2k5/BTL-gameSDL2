...
const string S_SCORE_TEXT = "Score: ";
	...
	int screenWidth, int screenHeight, const char* windowTitle)
{
    ...
    {
        cout << "MIX AUDIO ERROR"<<Mix_GetError()<<endl;
    }
...
void renderGamePlay(SDL_Renderer* renderer, const Game& game, Gallery* gallery)
{
    ...
    CreateGameText(renderer, to_string(game.score), 150, 600);
    CreateGameText(renderer, "Live: ", 240,600);
    CreateGameText(renderer, to_string(game.live),360,600);
...
Mix_Chunk* loadSound(string soundname)
{
    if(Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) == -1)
    {
        cout << "MIX ERROR! " << Mix_GetError() << endl;
    }

    string path = "audio/" + soundname; // Construct complete path
    Mix_Chunk* chunk = Mix_LoadWAV(path.c_str()); // Load sound using complete path

    if (chunk == NULL){
        cout<<"ERROR loading file: " << path << ", chunk is null: " << Mix_GetError() << endl;
    }
    return chunk;
}
